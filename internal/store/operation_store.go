...
func (s *OperationStore) AppendOperation(ctx context.Context, op crdt.Operation) error {
	switch o := op.(type) {
	case crdt.InsertOperation:
		...
		if err != nil {
			return fmt.Errorf("store: appending insert operation (doc=%s): %w", o.DocumentID, err)
		}
		return nil
	...
	case crdt.DeleteOperation:
		...
		if err != nil {
			return fmt.Errorf("store: appending delete operation (doc=%s): %w", o.DocumentID, err)
		}
		return nil

	default:
		return fmt.Errorf("store: unsupported operation type %T", op)
	}
}
